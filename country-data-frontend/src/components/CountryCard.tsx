...
export default function CountryCard({ countryUrl, countryName, region, code }: CountryCardType) {
  return (
    <div className="max-w-xs sm:max-w-sm lg:max-w-md m-3 rounded overflow-hidden transition duration-300 ease-in-out hover:shadow-lg dark:hover:shadow-black/30">
      <Link href={`/countries/code/${code}`}>
        <img
          className="w-full h-48 sm:h-64 object-cover transition duration-300 ease-in-out hover:scale-110"
          src={countryUrl}
          alt={countryName}
        />
      </Link>
